...
\newglossaryentry{g_Objective(OKR)}{
	name={Objective (OKR)},
	description={
		Der Begriff \textbf{"Objective"} bezieht sich in diesem Kontext auf die \gls{OKR}-Logik. Im Gegensatz zum militärischen Gebrauch im Englischen ist es hier ein wünschenswerter Zustand, der nicht zwingend eine eindeutige Zielerreichung voraussetzt. Dieser Zustand kann kontinuierlich angepasst und überprüft werden. Ein Beispiel: \textit{Mein Ziel ist, dass Deutschland ein demokratisches Land ist.} Was genau unter Demokratie verstanden wird, kann zu Beginn klar definiert sein, sich aber im Laufe der Zeit ändern und mit neuen Erkenntnissen abgeglichen werden.
