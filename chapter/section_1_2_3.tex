\begin{comment}
	...
	### Research Sources
	- Person-Course, MLOps for SDK v2,v1
...
\begin{comment}
	Using CLI
	\paragraph{*Remotely with a local setup}
	\href{https://learn.microsoft.com/en-us/cli/azure/install-azure-cli-windows?tabs=azure-cli}{Installation reference, then see above command to verify}
\end{comment}

\begin{comment}
	So the goal is, that to use local compute, to run an experiment.
	
	Reference: https://learn.microsoft.com/en-us/azure/machine-learning/how-to-configure-environment?view=azureml-api-2#local-computer-or-remote-vm-environment
	
	-> Find a enviroment setup, to use local compute ressources, however track everthing ... hahaha
	- https://dev.to/jakewitcher/using-env-files-for-environment-variables-in-python-applications-55a1
	- Open Emmy_ML_One
	- Open MLDevOpsML Python Repo
	
	.env lerning (but short)
	
	Follow the general conda VEN setup
	- creating conda
	- installing jupter notebook (IPython)
	- installing Azure ML SDK
	- create a AzureML configuration file (Note: This works only, for using local compute), installing it on the compute instance, it prohibited
	- Creating a conda file
\end{comment}
