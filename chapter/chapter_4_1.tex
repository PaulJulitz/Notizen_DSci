...
\textbf{Changelog:} \\
\begin{itemize}
    \item \textit{v1.2.2} Neues Prinzip: \nameref{subsec_LeitendePrinzipien_Vier}
    \begin{itemize}
        \item Anpassung der Begrifflichkeiten: Strategie zu Rubric. 
        \item Neu Sortierung der Prinzipien Reihenfolge. 
        \item \diffdate{2025}{09}{15}{2025}{10}{07}
    \end{itemize}

    \item \textit{v1.2.1} Kleine Formatverbesserungen
    \begin{itemize}
        \item Änderungen und Verbesserungen der Struktur und Lesbarkeit der \gls{g_Rubric} vorgenommen.
        \item Glossary- und Acronym-Verbesserung durch Filterung nicht relevanter Einträge.
        \item Hinzufügen eines Abstract.
        \item \diffdate{2024}{09}{15}{2024}{09}{18}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Erweiterungen wurden hinzugefügt, um zusätzliche Aspekte der Familien-\gls{g_Rubric} zu berücksichtigen.
        \item \diffdate{2024}{06}{10}{2024}{06}{20}
    \end{itemize}
    ...
    \begin{itemize}
        \item Diese Version legte den Grundstein für die NFM-\gls{g_Rubric}. 
...
Diese \gls{g_Rubric} - \gls{p_NFM} - zielt darauf ab, dieses Risiko zu minimieren, damit eine Familie tatsächlich das Schönste im Leben sein kann.\\


Die \gls{g_Rubric} gliedert sich in drei Teile:
\begin{itemize}
	...
	\item \textit{\nameref{sec:Erreichung}}, die die Frage klärt, was getan werden kann oder welche Konzepte Anwendung finden, um die \gls{g_Objective(OKR)} zu erreichen, und damit eine ausdifferenzierte Entscheidungshilfe darstellen.	\item und \textit{\nameref{sec:NFMEpilog}}, welche nicht Teil der \gls{g_Rubric} ist, jedoch Themen aufgreift, die im Bezug zu \gls{p_NFM} stehen.
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[scale = 0.4]{attachment/chapter_OWN/Scc010.png}
	\caption{NFM \gls{g_Rubric} Übersicht}
\end{figure} 


%Um \gls{p_NFM} eine stabile Familie zu bieten, auf welcher sie selbst später ihr eigenes Leben fußt, und für uns als Familie 

% One of the greates gift I can ofer them, is that rock, that foundation, to which build there life._
\pagebreak

\section{Objectives} 
%NFM; \gls{g_Rubric} Papier Name, Kategorie Name für das NFM

\subsection{Die Vier}

Die folgenden \gls{g_Objective(OKR)} stellen die ersten Ebenen des \gls{g_Rubric} da. Die jeweiligen Unterziele stellen ebenso \gls{g_Objective(OKR)} dar, welche die Zielausrichtung tiefer definieren.
...
Diese \gls{g_Objective(OKR)} dienen sowohl der Orientierung und Ausrichtung für langfristige Entscheidungen als auch dazu, in wichtigen Momenten bei der Priorisierung zu helfen.\\


Das erste \gls{g_Objective(OKR)}, \NFMOOne und \NFMOTwo, basiert auf der Überlegung, dass wir als \glspl{p_BFM} eine gesellschaftliche Verantwortung tragen, keine Last für die Gesellschaft zu sein. Im Idealfall ist \gls{p_NFM} eine Bereicherung für Familie und Gesellschaft, oder zumindest keine Belastung.\\
...
Der Unterschied zwischen \NFMOOne und \NFMOFour liegt darin, dass es einen Unterschied zwischen der Integration in die Gesellschaft und der Familie und der Verwirklichung des eigenen Lebens gibt. Das zweite \gls{g_Objective(OKR)} konzentriert sich darauf, als \gls{p_BFM} die Möglichkeiten aufzuzeigen und gelegentlich \gls{p_NFM} über emotionale Hürden zu begleiten, damit sie selbst entscheiden kann, ob sie diese weiterverfolgt.

\subsection{Analyse} \label{sec:Risiko_EKB}
Der folgende Abschnitt beschäftigt sich damit, welche Risiken in der Familien-Dynamik bestehen. Besonders wird der Schwerpunkt betrachtet, wie erfolgreich die gesetzten \gls{g_Objective(OKR)} sich realisieren lassen und welche gegenseitigen Abhänigkeiten existieren oder im Wege stehen.\footnote{
	Wie in der näheren Erläuterung von \gls{g_Objective(OKR)} beschrieben. Die gesetzten Objectives im Bezug zu \gls{p_NFM} haben kein Zeitparameter.
}
...
\subsubsection{Conundrum} Die Annahme besteht, dass Eingriffe am besten strategisch realisiert werden\footnote{
	Wichtig: Dies ist unabhängig davon, ob das \gls{g_Objective(OKR)} damit erreicht wird. Es geht nur um die Veränderung des Verhaltens von \gls{p_NFM}, nicht die interne Überzeugung.
...
\begin{itemize}
	\item Ein Risiko ist, dass die Positionierung von \gls{p_NFM} trotz des \gls{g_Objective(OKR)} nicht erreicht wird, weil der Wunsch besteht, diese Positionierung in der \gls{p_ER} nicht zu verlieren. 
...
Unter Betrachtung dieser Risikoanalyse, wird im nächsten Abschnitt adressiert, wie die \gls{g_Objective(OKR)} unter der Betrachtung der Risiken erreicht werden können.
...
Wie weit der Transit vollzogen ist und wie weit die drei \gls{g_Objective(OKR)} vorangeschritten sind, wird sich nicht eindeutig bestimmen lassen oder von allen gleich aufgefasst werden. Um dies als Familie permanent auszuloten, benötigt es Raum zur Debatte. Vor allem für \gls{p_NFM} sehen wir es als wichtig an, dass dieser Raum gegeben wird, um
\begin{itemize}
...
\subsection{Homogen in der Sache, heterogen als Team – \gls{p_HoZHeiT}}
\label{subsec_LeitendePrinzipien_Vier}

\subsubsection{Die Vorteile}  
Es ist weder nötig noch sinnvoll, in jeder Situation identisch zu handeln. Unterschiedliche Schwerpunkte oder Entscheidungen sind akzeptabel und können eine Bereicherung sein, indem sie neue Perspektiven eröffnen. Selbst wenn Unterschiede neutral bleiben, sichern sie Freiraum und Eigenständigkeit.

\textbf{Unser Anspruch}  
Wichtig ist nicht Einheitlichkeit im Handeln, sondern Kohärenz im Ziel. Der gemeinsame Rahmen von \gls{p_NFM} erlaubt es uns, verschiedene Wege in ein gemeinsames Ziel einzubetten.

\subsubsection{Abfederung}

Unterschiedliche Entscheidungen können …  
\begin{itemize}
    \item \textbf{positiv} inspirierend und bereichernd sein,  
    \item \textbf{neutral} als valide Alternative bestehen,  
    \item \textbf{negativ} die gemeinsame Linie belasten.  
\end{itemize}

\textbf{Hilfsperspektive}  
Wie bei einer Trennung können wir nicht in die Ad-hoc-Entscheidungen des anderen eingreifen. Unser Ziel ist es, trotzdem gemeinsam \NFMOThree zu verfolgen. Transparenz, Freiraum und klare Kommunikation sichern Handlungsfähigkeit und Ausrichtung.

\paragraph{Davor – Make your wishes known!}  
Frühzeitiges Teilen von Wünschen und Intentionen entschärft Konflikte. Wichtig: Gleiche Werte führen nicht automatisch zu gleichen Schlussfolgerungen.

\paragraph{Währenddessen – Wer vor Ort ist, entscheidet}  
Grundsatz: Wer in der Situation mit \gls{p_NFM} ist, trifft die Entscheidung – auch wenn beide anwesend sind und keine Zeit zur Diskussion bleibt.

\paragraph{Danach – Diskussion bleibt unser Ziel}  
Unterschiedliche Reaktionen lassen sich nicht verhindern. Entscheidend ist die Aufarbeitung: den Schaden klein halten und zeitnah gemeinsam reflektieren, siehe \ref{subsec_LeitendePrinzipien_Diskussionskultur}