...
%%%
    ...
    \node[text width=10cm, align=center] at ($(current page.center)+(4.5,-6)$) {
        { % Text wrapper will take into account single letters size. For example "p" needs more space in the secound line, then "a".
            \fontsize{30}{40} \selectfont  Best  \\ [9pt]  Solution
        } 
    };
    % Section 2
    \node[text width=10cm, align=center] at ($(current page.center)+(0,-2.5)$) {
        ...
        {
            \fontsize{30}{40} \selectfont A Great Team 
        }
    };
    % Section 2
	\node[text width=10cm, align=center] at ($(current page.center)+(0,-4.2)$) {
		
		{
			\fontsize{30}{40} \selectfont Vision
		}
	};
    % Section 3
    \node[text width=10cm, align=center] at ($(current page.center)+(-4.5,-6)$) {
        { % Text wrapper will take into account single letters size. For example "p" needs more space in the secound line, then "a".
            \fontsize{30}{40} \selectfont Alignment \\ [9pt] $\&$ Decision
        }
    };
    ...
    % Family-Stability
    \draw[ultra thick,gray] (7,-15.5) -- (8.5,-15.5);
    % Family-Stability
	\draw[ultra thick,gray] (7,-17.2) -- (8.5,-17.2);
    ...
    \node[text width=12cm,align=center] at ($(current page.center)+(0,-10)$) {
        ...
        {
            We sacrifice a bit of comfort for long-term success.
        }	 
 };
